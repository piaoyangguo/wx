layui.define(['mobile', 'rmlib', 'protobuf'], function (exports) {
    var lib = layui.rmlib;
    var $ = layui.zepto;
    ...
    var im = {
        ...
        getToken: function (uid, callback) {
            ...
            if (typeof conf.token == 'string'){
                log('使用配置token字符，并将token保存到本地');
                token.save(conf.token);
                callback(conf.token);
                return;
            }
        ...
        sendMsg: function (data) {
            ...
            var msg = {
                content: mine.content,
                extra: {"username": mine.username, "avatar": mine.avatar, "id": id, "type": to.type},
            }
            var detail = new lib.TextMessage(msg);
            //发送消息
            RongIMClient.getInstance().sendMessage(conversationType, targetId, detail, {
                onSuccess: function (message) {
                    log('发送消息成功');
                    //$.post('/', { userId: mine.id, to: targetId, type: msg.type, content: msg.content }, function (res) {
                    //    log(res);
                    //})
                },
                onError: function (errorCode) {
                ...
                onReceived: function (message) {
                    ...
                    switch (message.messageType) {
                        ...
                        case RongIMClient.MessageType.TextMessage:
                            ...
                            var extraobj = message.content.extra;
                            if (typeof (extraobj) == "string") {
                                extraobj = JSON.parse(extraobj);
                                var getmsg = {
                                    "username": extraobj.username,
                                ...
                                var getmsg = {
                                    "username": message.content.extra.username,
                                    "avatar": message.content.extra.avatar,
                                    "id": message.content.extra.id,
                                    "type": message.content.extra.type,
                                    "content": message.content.content
                                }
                            }
                            conf.layim.getMessage(getmsg);
                            break;
